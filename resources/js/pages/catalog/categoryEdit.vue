...
export default {
    ...
    methods: {
        getFile(event) {
            ...
            this.isImageString = false;
        },
        async formSubmit(e) {
            e.preventDefault();
            this.category.isImageString = this.isImageString;
            const formData = commonFunction.formdata(this.category);
            formData.append("_method", "PUT");
            try {
                const { data: updateResponse } = await APISERVICE.post(
                    `admin/category/${this.category.id}`,
                    formData
                );
        ...
        setFile() {
            this.categoryImg =
                "/images/uploads/category/" + this.category.image;
